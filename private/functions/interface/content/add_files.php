...
function saveThumbnail($image, $filename, $image_file_type) {
    ...
    $file_path = base_path(IMAGE_UPLOAD_PATH."thumbnails/".$filename);
    switch ($image_file_type) {
        case "jpg":
        case "jpeg":
...
function uploadMedia($files, $key, $db, $table, $image_file_type = null) {
    ...
    $upload_path = $table == "images" ? base_path(IMAGE_UPLOAD_PATH) : base_path(AUDIO_UPLOAD_PATH);
        ...
        if ($image) {
            ...
            try {
                if ($image_size[0] > MAX_IMAGE_WIDTH) {
                    if (!resizeImage($image, base_path(IMAGE_UPLOAD_PATH.$files["name"][$key]), $image_file_type)) {
